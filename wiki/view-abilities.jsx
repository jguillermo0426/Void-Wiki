...
(function () {
  ...
  const { go, PageHead, Empty, typeAccent } = window.VUI;
        ...
        {list.length === 0 ? <Empty label="No abilities match." /> : (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(340px, 1fr))', gap: 16 }}>
            {list.map((a, i) => {
              ...
              return (
                <div key={i} style={{ padding: 18, borderRadius: 14, background: a.rare ? 'linear-gradient(160deg, #1a1140, #0e0b1f)' : '#0e0b1f', border: `1px solid ${a.rare ? '#3a2f6e' : '#221d3a'}` }}>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {mons.map(dx => { const m = byDex(dx); if (!m) return null; const accent = typeAccent(m.types); return (
                      <button key={dx} onClick={() => go('#/pokemon/' + dx)} style={{ cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 6, padding: '4px 10px 4px 6px', borderRadius: 999, background: accent.borderFill, border: '1px solid transparent' }}>
