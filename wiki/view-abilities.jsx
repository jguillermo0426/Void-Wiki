/* Pokémon Void — Abilities view. window.VIEWS.Abilities */
...
(function () {
  ...
  window.VIEWS.Abilities = function Abilities() {
    ...
    return (
      <div>
        <PageHead kicker="ABILITY INDEX" title="Abilities" sub="Passive powers that shape every battle. Team Void's signature mons lean on Vacuum and Void-flavored traits to starve opponents of recovery." />
            ...
            {list.map(a => (
              <div key={a.name} style={{ padding: 18, borderRadius: 14, background: a.rare ? 'linear-gradient(160deg, #1a1140, #0e0b1f)' : '#0e0b1f', border: `1px solid ${a.rare ? '#3a2f6e' : '#221d3a'}` }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  {a.mons.map(dx => { const m = byDex(dx); if (!m) return null; return (
